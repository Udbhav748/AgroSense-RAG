...
| Encryption | ⚠️ | Transport: TLS via the Caddy overlay is now the documented default deploy path (`docs/OPERATIONS.md`), but still needs a real domain and so can't be unconditional — the no-TLS fallback still exists (LOCAL HTTPS is documented/configurable, not a live public deployment). At-rest: **application-level encryption is now genuinely wired into real storage**, not just a standalone primitive. `app/services/postgres_session_store.py` encrypts `ChatTurn.content` (the actual text of every stored user query/assistant answer — the most sensitive field this app persists) with AES-256-GCM (`app/core/encryption.py`) before writing, and decrypts on read; the `session_id` is bound as authenticated associated data (a ciphertext from one session can't be decrypted under another, even with the right key). Key comes from `Settings.encryption_key_b64` (`.env`'s `ENCRYPTION_KEY_B64`), never hardcoded/committed; missing/wrong/tampered key fails closed (never falls back to plaintext). Legacy plaintext rows (written before this integration) remain readable without a forced migration; every new write is unconditionally encrypted. 12/12 integration tests in `tests/test_postgres_session_store_encryption.py` against a real (SQLite) database through the actual repository, covering encrypted write, decrypt round-trip, wrong key, tampering, missing key, session-isolation, and legacy-row backward compatibility — plus the underlying primitive's own 9/9 tests. **Still not encrypted**: uploaded document files, FAISS index/metadata, and `ChatSession.title` (currently unpopulated by any code path) — disclosed, not attempted this pass; would need their own migration story since they're read directly by PyMuPDF/OCR/S3-sync. Platform-level encryption (an encrypted EBS volume) remains unconfigured since no live deployment exists. This is one security control, not a compliance certification (not GDPR/HIPAA-compliant merely because this exists). Evidence: `eval/module10/reports/encryption_at_rest_integration_*.json`. |
