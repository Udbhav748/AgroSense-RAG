...
| Encryption | ⚠️ (expanded 2026-09-21) | Transport: TLS via the Caddy overlay is now the documented default deploy path (`docs/OPERATIONS.md`), but still needs a real domain and so can't be unconditional — the no-TLS fallback still exists (LOCAL HTTPS is documented/configurable, not a live public deployment). At-rest: **application-level encryption covers two real, sensitive text columns**, not just one standalone primitive. `app/services/postgres_session_store.py` encrypts `ChatTurn.content` (the actual text of every stored user query/assistant answer) and `app/services/session_repository.py` encrypts `ChatSession.title` (populated verbatim from a user's first message — genuinely sensitive, not a placeholder) — both via shared `encrypt_text_field`/`decrypt_text_field` helpers in `app/core/encryption.py` (AES-256-GCM), with `session_id` bound as authenticated associated data on both fields (a ciphertext from one session can't be decrypted under another, even with the right key). Key comes from `Settings.encryption_key_b64` (`.env`'s `ENCRYPTION_KEY_B64`), never hardcoded/committed; missing/wrong/tampered key fails closed (never falls back to plaintext) on both fields. Legacy plaintext rows (written before each field's integration) remain readable without a forced migration; every new write is unconditionally encrypted. 33 tests total (`tests/test_encryption.py`, `tests/test_postgres_session_store_encryption.py`, `tests/test_session_repository_encryption.py`) against a real (SQLite) database through the actual repositories, covering encrypted write, decrypt round-trip, wrong key, tampering, missing key, session-isolation, legacy-row backward compatibility, and plaintext-leakage-marker detection. **Still not encrypted**: uploaded document files on disk, FAISS index/metadata (chunk text), and feedback records — each disclosed with a specific technical reason (see `backend/eval/module10/reports/encryption_at_rest_final_*.json`'s coverage matrix), not silently omitted; would need a decrypt-to-tempfile/decrypt-on-every-retrieval redesign. Platform-level encryption (an encrypted EBS volume) remains unconfigured since no live deployment exists. This is one security control, not a compliance certification (not GDPR/HIPAA-compliant merely because this exists). Evidence: `eval/module10/reports/encryption_at_rest_final_*.json` (current), `eval/module10/reports/encryption_at_rest_integration_*.json` (historical, `ChatTurn.content` only). |
