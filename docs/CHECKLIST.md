...
| Encryption | ⚠️ | Transport: TLS via the Caddy overlay is now the documented default deploy path (`docs/OPERATIONS.md`), but still needs a real domain and so can't be unconditional — the no-TLS fallback still exists (LOCAL HTTPS is documented/configurable, not a live public deployment). At-rest: **application-level primitive now implemented and tested** — `app/core/encryption.py` (AES-256-GCM via `cryptography`, key from `ENCRYPTION_KEY_B64`, never hardcoded; 9/9 tests in `tests/test_encryption.py`: round-trip, wrong-key failure, tampered-ciphertext failure, missing-key failure, associated-data binding). **Not yet wired into the upload/storage pipeline** (documents/FAISS index/metadata are still written as plaintext) — that integration needs its own migration story for already-uploaded plaintext data and is a separate, larger change, not attempted this pass. Platform-level encryption (an encrypted EBS volume) remains unconfigured since no live deployment exists. Evidence: `eval/module10/reports/encryption_at_rest_20260920T005611Z.json`. |
